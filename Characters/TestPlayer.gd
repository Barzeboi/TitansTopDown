		...
		WITH_GUN:
			if _input_vector.length_squared() > 0:
				if _input_vector.x != 0 and _input_vector.y != 0:
					...
					if _input_vector.y > 0:
						if _input_vector.x > 0:
							_animated_sprite.play("shooting_down_right")
						else:
							_animated_sprite.play("shooting_down_left")
					else:
						if _input_vector.x > 0:
							_animated_sprite.play("shooting_up_right")
						else:
							_animated_sprite.play("shooting_up_left")
				elif _input_vector.x != 0:
					if _input_vector.x > 0:
						_animated_sprite.play("shooting_right")
					else:
						_animated_sprite.play("shooting_left")
				else:
					if _input_vector.y > 0:
						_animated_sprite.play("shooting_down")
					else:
						_animated_sprite.play("shooting_up")
